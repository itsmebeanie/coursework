...
public class GlassBeads {
	...
	static SuffixTree.Node dfs(SuffixTree.Node node, int n) {
		if (node.b - node.a + 1>= n){
			return node;
		}
		for (SuffixTree.Edge e : node){
			SuffixTree.Node temp = dfs(e.end, n);
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		while (T-- > 0) {
			...
			SuffixTree st = new SuffixTree(s + s);
			SuffixTree.Node n = dfs(st.getRoot(), s.length());
	...
	public static class SuffixTree {
		...
		public class Edge {
			/**
			 * An inclusive range [a,b] of indices into str representing the
			 * edge label.
			 */
			public int a, b;
		...
		public class Node implements Iterable<Edge> {
			...
			private Node suffix;
			/**
			 * An inclusive range [a,b] of indices into str representing a
			 * prefix leading to this node
			 */
			public int a, b;
		...
		@Override
		/**
		 * Pretty prints the tree. For each node it prints the prefix leading to
		 * that node in quotes. Then it prints each edge leaving the node
		 * indexed by first letter, the edge label, the inclusive range of
		 * indices, and the length of the label.
		 */
		public String toString() {
		...
		private StringBuilder prettyFormat(StringBuilder sb, Node n, StringBuilder tab) {
			...
			for (Edge e : n) {
				char c = e.getFirst();
				sb.append(tab).append(c).append(" : ").append(e).append(" = ").append(e.a).append(',').append(fix(e.b))
		...
		private void setPrefix(Node n, Edge e, int len) {
			for (Edge edge : n)
				setPrefix(edge.end, edge, len + Math.min(edge.b, last) - edge.a + 1);
			if (e == null) {
				n.a = 0;
				n.b = -1;
			} else {
		...
		private Node testAndSplit(Node s, int k, int p, char c) {
			if (k > p)
				return s == sentinel ? null : s.get(c) == null ? s : null;
			Edge e = s.get(str.charAt(k));
			if (c == str.charAt(e.a + p - k + 1))
				return null; // check if char after implicit node is c
			Node r = new Node();
		...
		private Node canonize(Node s, int[] k, int p) {
			...
			while (e.b - e.a <= p - k[0]) {
				...
				s = e.end;
				if (k[0] <= p)
					e = s.get(str.charAt(k[0]));
			}
			return s;
		}

		/**
		 * Can be used to test the suffix tree on a given string given as a
		 * cmdline argument.
		 */
